...
namespace Flexion
{
    public class Piece
    {
        ...
        public double[] CalculateI()
        {
            ...
            {
                double[] power = AdditionalMath.OperationDoubleArray(Couches[i].Hauteur(Longueur, (double[])Xs.Clone()).ToArray(), 3, AdditionalMath.Operation.Puissance);
                double[] divise = AdditionalMath.OperationDoubleArray(power, Couches[i].Base(Longueur, Eref, (double[])Xs.Clone()), AdditionalMath.Operation.Fois);
                Ix[i] = AdditionalMath.OperationDoubleArray(divise, 12, AdditionalMath.Operation.Divisé);
            }

            for (int i = 0; i < Couches.Count; i++)
            {
                double[] P1 = AdditionalMath.OperationDoubleArray(CalculateNx(i, Xs.Length), Ns(), AdditionalMath.Operation.Moins);
                double[] P2 = AdditionalMath.OperationDoubleArray(P1, 2, AdditionalMath.Operation.Puissance);
                double[] P3 = AdditionalMath.OperationDoubleArray(Couches[i].Surface(Longueur, Eref, (double[])Xs.Clone()).ToArray(), P2, AdditionalMath.Operation.Fois);
        ...
        public double[] Ns()
        {
            ...
            for (int i = 0; i < Couches.Count; i++)
            {
                double[] ajout = AdditionalMath.OperationDoubleArray(Couches[i].Surface(Longueur, Eref, (double[])Xs.Clone()).ToArray(), Nx[i], AdditionalMath.Operation.Fois);
            ...
            //calcule de Ns
            return AdditionalMath.OperationDoubleArray(divise, divisant, AdditionalMath.Operation.Divisé);
        }
        ...
        public double[] CalculateNx(int i, int length)
        {
            ...
            for (int j = 0; j <= i; j++)
            {
                if (j == i)
                {
                    double[] add = AdditionalMath.OperationDoubleArray(Couches[j].Hauteur(Longueur, (double[])Xs.Clone()).ToArray(), 2, AdditionalMath.Operation.Divisé);
                    Nx = AdditionalMath.OperationDoubleArray(Nx, add, AdditionalMath.Operation.Plus);
                }
                else
                {
                    Nx = AdditionalMath.OperationDoubleArray(Nx, Couches[j].Hauteur(Longueur, (double[])Xs.Clone()).ToArray(), AdditionalMath.Operation.Plus);
                }
            }
            return Nx;
        }
